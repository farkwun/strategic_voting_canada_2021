...
class SVDatabase(Database):
    def district_exists(self, district_id):
        sql = "SELECT id FROM districts where id = '{district_id}'".format(district_id=district_id)
        self.query(sql)
        return len(self.fetchall()) != 0

    def get_district_from_id(self, district_id):
        sql = "SELECT * FROM districts WHERE id = '{district_id}'".format(district_id=district_id)
        self.query(sql)
